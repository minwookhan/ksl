...
*** 1. [cite/t:@mfc_server] 기능 개요
=@mfc_server=는 *Windows MFC 기반의 수어 인식 서버 애플리케이션*입니다. 이 서버는 클라이언트로부터 영상 스트림(프레임 및 스켈레톤 데이터)을 받아, 내부적으로 파이썬 AI 모델(ACR)과 방대한 엑셀 기반 수어 DB를 활용해 수어 문장을 추론하고 결과를 반환하는 역할을 합니다.

주요 기능 모듈은 다음과 같습니다.

- *메인 애플리케이션 (UI & Control)*: =KSLDBManagerDlg=가 메인 컨트롤러 역할을 하며, UI 표시, AI 스레드 관리, DB 로딩 등을 총괄합니다.
- *통신 레이어 (gRPC)*: =gRPCThread=를 통해 =0.0.0.0:50051= 포트에서 서버를 구동하며, 클라이언트로부터 스트리밍되는 영상 데이터를 수신합니다.
- *데이터베이스 (Excel DB)*: =Excel_DB= 클래스를 통해 수어 영상, 표제어(Gloss), 코퍼스(Corpus), 체인지포인트(CP) 정보를 담은 대형 엑셀 파일(=ETRI_KSL_Dictionary...xlsx=)을 메모리에 로드하고 관리합니다.
- *영상 처리 및 AI 추론*:
  - *Python 연동*: =pybind11=을 사용하여 C++ 내부에서 Python 스크립트(=ACR.detect=, =ACR.mp_detect=)를 직접 호출, 3D 손 메쉬 복원(MANO) 및 특징 추출을 수행합니다.
  - *유사도 검색*: 입력된 프레임의 손 모양/방향/위치를 DB에 저장된 체인지포인트(CP)들과 비교(=FindSimilarCPs=)하여 가장 유사한 표제어 후보를 찾습니다.
  - *문장 추론*: =InferSentence= 모듈을 통해 개별 프레임에서 인식된 표제어들의 시퀀스를 분석하여, 가장 확률이 높은 자연어 문장을 도출합니다.
- *알고리즘*: =ChangePointDetector=를 통해 동작의 변화 시점을 감지하고, =HandShapeDiff= 등을 통해 손 모양의 차이를 수치화하여 유사도를 계산합니다.

--------------

*** 2. gRPC 데이터 수신 이후 처리 과정
클라이언트가 =SequenceService::SendFrames= RPC를 호출하여 데이터를 스트리밍하기 시작하면, 서버(=gRPCThread.h= 및 =KSLDBManagerDlg.cpp=)에서는 다음과 같은 단계로 처리가 이루어집니다.

**** *Step 1: 데이터 수신 및 디코딩 (gRPC Thread)*
- *위치*: =gRPCThread.h= -> =SequenceServiceImpl::SendFrames=
- 클라이언트로부터 =Frame= 메시지(이미지 바이너리 + MediaPipe 포즈 좌표)를 스트리밍으로 수신합니다.
- 수신된 =Frame=의 바이너리 데이터를 OpenCV =cv::Mat= 이미지로 디코딩하고, 3D 포즈 좌표(=pose3d=)를 벡터로 변환합니다.

**** *Step 2: AI 스레드 처리 요청 (Sync)*
- *위치*: =gRPCThread.h= -> =KSLDBManagerDlg.cpp=
- gRPC 스레드는 수신된 이미지와 포즈를 메인 다이얼로그(=pDlg=)의 멤버 변수에 설정합니다.
- =AI_MODE_HAND_DETECT_NEW_DB= 모드로 플래그를 설정하고 =SetEvent(pDlg->hAIStart)=를 호출하여 대기 중인 AI 전용 스레드를 깨웁니다.
- gRPC 스레드는 AI 처리가 끝날 때까지(=WaitForSingleObject=) 대기합니다.

**** *Step 3: 손 검출 및 특징 추출 (AI Thread)*
- *위치*: =KSLDBManagerDlg.cpp= -> =AIThread= 함수
- AI 스레드는 Python 모듈(=ACR.detect=)을 호출하여 입력된 이미지에서 *3D 손 메쉬(MANO Model)와 관절 정보*를 정밀하게 추출합니다.
- 추출된 정보(관절 각도, 손 방향 벡터, PCA 성분 등)를 C++ 구조체(=Result=)로 변환하고, 이를 바탕으로 현재 프레임의 *체인지포인트(CP) 데이터*를 구성합니다.
- 처리가 완료되면 =SetEvent(pDlg->hAIFinish)=를 호출하여 gRPC 스레드에게 알립니다.

**** *Step 4: 유사도 검색 및 후보군 누적 (gRPC Thread)*
- *위치*: =gRPCThread.h= -> =SequenceServiceImpl= Loop 내부
- AI 처리가 완료되면 gRPC 스레드가 다시 깨어납니다.
- 현재 프레임의 손 상태(양손/우세손/비우세손)를 판별합니다.
- =pDlg->FindSimilarCPs= 함수를 호출하여, 추출된 현재 손 특징과 *DB에 저장된 모든 체인지포인트(CP)*를 비교합니다.
- 유사도 점수(=SimilarScore=)가 높은 상위 후보(Gloss)를 찾아 =m_infered= 리스트에 시간 순서대로 누적합니다.

**** *Step 5: 문장 추론 및 응답 전송 (Stream 종료 후)*
- *위치*: =gRPCThread.h= -> =SequenceServiceImpl= (Loop 종료 후)
- 클라이언트의 스트리밍이 끝나면(=reader->Read= 종료), 누적된 =m_infered= 리스트(인식된 표제어 후보들)를 =pDlg->m_infer_sentence.InferCorpus=에 전달합니다.
- *InferCorpus*: 인식된 표제어들의 조합, 등장 빈도, 순서 등을 고려하여 코퍼스 DB(=m_CorpusVideoDB=) 내에서 가장 적합한 문장을 찾습니다.
- 최종적으로 인식된 문장(예: "안녕하세요")을 =SubmitResultResponse=에 담아 클라이언트에게 반환합니다.
