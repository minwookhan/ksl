...
* 데이터 흐름 분석-3
=mfc_client_src= 내의 코드(=gRPCThread_.h=의 =SequenceClient::SendFrames=, =gRPCFileClientDlg.cpp=, =HandTurnDetector.hpp= 등)를 분석하여, *하나의 프레임 이미지*가 입력되었을 때 *KeyFrame(전송할 유의미한 프레임)으로 결정되는 과정*을 체계적으로 설명합니다.

이 프로세스는 크게 *입력 및 전처리*, *AI 골격 추출*, *모션 분석(Optical Flow)*, *손 궤적 분석(Turn Detection)*, *최종 결정*의 5단계로 진행됩니다.

--------------

*** 1. 입력 및 전처리 (Preprocessing)
비디오나 카메라로부터 한 장의 이미지를 획득하고 분석 대상 영역(ROI)을 설정하는 단계입니다.

- *함수 위치*: =gRPCThread_.h= -> =SequenceClient::SendFrames=
- *입력*:
  - =cv::Mat img=: 원본 프레임 (예: 1920x1080, BGR, =CV_8UC3=)
  - =cv::Rect m_roi=: 사용자가 설정한 관심 영역
- *처리*:
  - =img=에서 =m_roi= 영역을 잘라내어(=roi=) =pDlg->m_cap_img=에 저장합니다.
  - ROI의 너비가 320px 이상이면 320px 기준으로 리사이즈(축소)합니다.
- *출력*:
  - =pDlg->m_cap_img=: 분석용 이미지 (예: 320xN, BGR, =CV_8UC3=)

*** 2. AI 골격 추출 (Pose Estimation)
MediaPipe를 이용해 인체 랜드마크(스켈레톤)를 추출합니다.

- *함수 위치*: =gRPCFileClientDlg.cpp= -> =AIThread= (Python 임베딩 호출)
- *입력*:
  - =pDlg->m_cap_img=: 전처리된 이미지
- *처리*:
  - Python =ACR.mp_detect.mediapipe_pose_func= 호출.
  - MediaPipe Pose 솔루션을 수행하여 33개의 3D 랜드마크 좌표를 얻습니다.
- *출력 (주요 변수)*:
  - =pDlg->m_sgcp_vtSkeletonMP=: =std::vector<cv::Point3f>=
    - *크기*: 33 (Pose Landmarks 개수)
    - *값*: 0.0 ~ 1.0 사이의 정규화된 좌표 (x, y, z)
    - /예: 손목 좌표(인덱스 15, 16)가 (0.45, 0.62, -0.1) 형태로 저장됨./

*** 3. 모션 강도 분석 (Optical Flow)
프레임 간의 픽셀 변화량을 계산하여 움직임이 멈추었는지(정지 동작) 판단합니다.

- [[id:3e43cf43-c0b8-4986-a449-64f2e076ed6a][Optical Flow 설명]]
- *함수 위치*: =gRPCFileClientDlg.cpp= -> =GetMotionVauleWithOpticalFlow=
- *입력*:
  - =cv::Mat roi=: 현재 프레임의 ROI 이미지
  - =static cv::Mat prev=: 이전 프레임 이미지
- *처리*:
  - =calcOpticalFlowPyrLK=를 사용하여 특징점들의 이동 벡터를 계산.
  - 이동 벡터들의 평균 길이(Euclidean distance)를 산출.
- *출력*:
  - =avgMotion=: =double=
    - *값*: 평균 픽셀 이동량 (예: 0.1 ~ 20.0)
    - /값이 작을수록(=OPTICALFLOW_THRESH= 미만) 동작이 멈춘 상태임을 의미./

*** 4. 손 궤적 방향 전환 감지 (Hand Turn Detection)
손이 움직이다가 방향을 급격히 바꾸는 지점(변곡점)을 감지합니다. 수어 동작의 시작이나 끝을 알리는 신호로 사용됩니다.

- *함수 위치*: =HandTurnDetector.hpp= -> =HandTurnDetector::update=
- *입력*:
  - =pos=: 손목 좌표 =cv::Point2f(x, y)= (정규화된 좌표, =m_sgcp_vtSkeletonMP=에서 가져옴)
  - =dt=: 프레임 간 시간 간격 =double= (예: 0.033 sec)
- *처리*:
  - 현재 속도 벡터와 이전 속도 벡터의 *각도 변화(=dtheta=)* 및 *속도 감소 비율(=ratio=)* 계산.
  - *조건*: 각도 변화 > 30도 (=angleRadTh_=) *AND* 속도 20% 이상 감소 (=speedRatioTh_ = 0.8=).
- *출력*:
  - =bool detected=:
    - =true=: 방향 전환 발생 (유효한 코너)
    - =false=: 일반적인 이동

*** 5. 최종 KeyFrame 결정 (Decision Logic)
위의 정보들을 종합하여 현재 프레임을 서버로 전송할지 결정합니다.

- *함수 위치*: =gRPCThread_.h= -> =SequenceClient::SendFrames= 내부 루프
- *판단 로직 (AND 조건)*:
  1. *손 상태 유효*: =hand_status != 0= (손이 인식됨)
  2. *동작 시작됨 (=speakstartflag=)*: 이전에 =HandTurnDetector=에 의해 손의 방향 전환이 한 번이라도 감지되어 =speakstartflag=가 1로 세트되어야 함.
  3. *정지 상태 (=avgMotion=)*: =avgMotion < OPTICALFLOW_THRESH= (0.45). 손이 움직이다가 멈춘 순간(KeyFrame 후보).
  4. *중복 방지 (=frozenflag=)*: =frozenflag == 0=. 최근에 프레임을 전송한 적이 없어야 함.
- *결과*:
  - 조건 만족 시:
    - =writer->Write(frame)=: 현재 프레임과 스켈레톤 정보를 gRPC로 전송.
    - =frozenflag = OPTICAL_FLOW_HOLD_FRAME= (3): 향후 3프레임 동안 전송 금지.
  - 조건 불만족 시:
    - 프레임 스킵 (전송하지 않음).
...
#+begin_src mermaid
graph TD
    A[입력 이미지 (Frame)] --> B[전처리 (Resize & ROI)]
    B --> C[AI Inference (MediaPipe)]
    C -->|Skeleton (33pts)| D{손 상태/위치 확인}

    B --> E[Optical Flow 계산]
    E -->|avgMotion| F{움직임 멈춤?}

    C --> G[HandTurnDetector]
    G -->|True/False| H{방향 전환(Turn) 감지?}

    H -- Yes --> I[SpeakStartFlag = 1]

    D --> J{최종 판단}
    F -- Yes (< 0.45) --> J
    I -- Yes --> J
    J -- 조건 만족 (Turn 후 멈춤) --> K[KeyFrame 전송 (gRPC Write)]
#+end_src
...
이 알고리즘은 MediaPipe를 통해 추출된 정밀한 포즈 데이터와 OpenCV의 옵티컬 플로우, 그리고 수어 동작의 특성을 고려한 커스텀 손 움직임 감지 (=HandTurnDetector=)를 결합하여, 동영상에서 의미 있는 수어 KeyFrame을 효율적으로 식별하고 전송하도록 설계되었습니다.
* mediapipe에서 데이터 가져오는 Cpp 내용
#+begin_quote
[cite/t:@repomix-output-gRPC_client.zip.md] 에서 mediapipe 자료를 얻거나 실행시키는 방법은?

#+end_quote

✦ repomix-output-gRPC_client.zip.md 파일 내에서 MediaPipe 데이터를 얻거나 실행시키는 방법은 C++ 애플리케이션에 Python 코드를 임베딩(Embedding)하는 방식으로 이루어집니다. 즉, C++ 코드가 직접 MediaPipe를 실행하는 것이 아니라, C++에서 Python 인터프리터를 초기화하고 Python 모듈을 호출하여 MediaPipe 추론을 수행하게 합니다.

다음은 MediaPipe 데이터를 얻고 실행하는 단계별 설명입니다.

1. Python 환경 및 pybind11 초기화 (C++ 측)

gRPCFileClientDlg.h 파일을 보면 pybind11 관련 헤더들이 포함되어 있습니다. 이는 C++ 애플리케이션 내에서 Python 인터프리터를 사용하고, C++와 Python 객체 간의 데이터 변환을 가능하게 합니다.

=gRPCFileClientDlg.h= 의 관련 코드:
...
10 namespace detail { 11 template<> 12 struct type_caster { 13 public: 14 PYBIND11_TYPE_CASTER(cv::Mat, _("numpy.ndarray")); 15 16 // 1. numpy.ndarray -> cv::Mat 변환 17 bool load(handle obj, bool) { 18 // ... (생략) ... 19 } 20 // 2. cv::Mat -> numpy.ndarray 변환 21 // ... (생략) ... 22 }; 23 } 24 }

- =<python.h>=: Python C API를 사용하기 위한 헤더입니다.
- =<pybind11/embed.h>=: C++ 애플리케이션에 Python 인터프리터를 임베딩하는 기능을 제공합니다.
- =type_caster<cv::Mat>=: cv::Mat (OpenCV 이미지) 객체를 Python의 numpy.ndarray로, 또는 그 반대로 변환하는 규칙을 정의합니다. 이를 통해 C++ 이미지 데이터를 Python 함수에 자연스럽게 전달할 수 있습니다.

2. [@2] MediaPipe 추론 실행 (AIThread 내 Python 호출)

gRPCFileClientDlg.cpp와 gRPCThread_.h 파일의 맥락을 보면 AIThread라는 별도의 스레드가 존재합니다. 이 스레드가 MediaPipe 추론을 담당하며, hAIStart 이벤트가 설정되면 동작을 시작합니다.

AIThread 내부에서는 Python 모듈을 임포트하고, 이미지 프레임을 Python 함수에 전달하여 MediaPipe 추론을 수행합니다.

추정되는 =AIThread= (또는 관련 함수)의 논리 흐름 (코드는 =zip.md=에 명시적으로 없음):

#+begin_example
1 // gRPCThread_ 스레드에서 비디오 프레임을 캡처한 후
2 // m_cap_img (cv::Mat 타입)에 현재 프레임을 저장합니다.
3
4 // AIThread (별도의 스레드)
5 UINT AIThread(LPVOID pParam) {
6     CgRPCFileClientDlg* pDlg = (CgRPCFileClientDlg*)pParam;
7
8     // Python 인터프리터 초기화 (한 번만 수행)
9     pybind11::scoped_interpreter guard{}; // GIL 확보
#+end_example

10 11 // ACR 모듈 로드 (실제 MediaPipe 로직을 담고 있는 Python 파일) 12 // 이 파일은 C++ 프로젝트 외부에 있거나, 임베딩된 Python 환경에서 접근 가능한 경로에 있어야 합니다. 13 pybind11::module_ acr_module = pybind11::module_::import("ACR.mp_detect"); // 예: ACR.py의 mp_detect 함수 14 15 while (true) { 16 // gRPC Thread가 이미지 처리를 요청할 때까지 대기 17 WaitForSingleObject(pDlg->hAIStart, INFINITE); 18 if (pDlg->m_exit.load()) break; // 종료 신호 19 20 // Python Global Interpreter Lock (GIL) 획득 21 // C++ 스레드에서 Python 함수를 호출하기 위해 필요 22 pybind11::gil_scoped_acquire acquire; 23 24 try { 25 // C++ 이미지 (m_cap_img)를 Python 함수에 전달 26 // type_caster 덕분에 cv::Mat이 자동으로 numpy.ndarray로 변환됨 27 pybind11::object result_pose_mp = acr_module.attr("mp_detect")(pDlg->m_cap_img); 28 29 // Python에서 반환된 MediaPipe 결과(스켈레톤 랜드마크)를 C++ 타입으로 캐스팅 30 // 예: std::vector<std::vector> 형태로 받음 31 pDlg->m_sgcp_vtSkeletonMP.clear(); 32 for (const auto& landmark_data : result_pose_mp.cast<std::vector<std::vector>>()) { 33 pDlg->m_sgcp_vtSkeletonMP.push_back(cv::Point3f(landmark_data[0], landmark_data[1], landmark_data[2])); 34 } 35 36 } catch (const pybind11::error_already_set &e) { 37 // Python 실행 중 발생한 오류 처리 38 OutputDebugString(e.what()); 39 } 40 41 // AI 추론 완료를 gRPC Thread에 알림 42 SetEvent(pDlg->hAIFinish); 43 } 44 return 0; 45 }

3. [@3] MediaPipe 데이터 얻기 (C++ 측)

MediaPipe 추론 결과는 Python ACR.mp_detect 함수에서 계산된 후, pybind11::object로 C++에 전달됩니다. C++ 측에서는 이 pybind11::object를 cast<std::vector<std::vector>>()와 같은 방식으로 C++의 std::vector<std::vector>로 변환합니다.

그리고 이 std::vector<std::vector>의 각 요소를 cv::Point3f 객체로 변환하여 최종적으로 CgRPCFileClientDlg 클래스의 멤버 변수인 m_sgcp_vtSkeletonMP (std::vector)에 저장됩니다. 이 m_sgcp_vtSkeletonMP가 C++ 코드에서 MediaPipe 추론 결과를 나타내는 자료 구조입니다.

요약: * 실행: C++ 코드 내 AIThread에서 pybind11::module_::import("ACR.mp_detect")를 통해 Python 모듈의 함수를 호출하여 MediaPipe 추론을 간접적으로 실행합니다. * 데이터 획득: Python 함수로부터 반환된 결과(일반적으로 랜드마크 좌표 리스트)를 pybind11::cast<...>()를 사용하여 C++의 std::vector<std::vector>로 변환하고, 다시 std::vector 형태의 m_sgcp_vtSkeletonMP에 저장하여 사용합니다.

#+begin_quote
ACR.mp_detect 로 가져온 데이터를 어떻게 처리하였는가?

#+end_quote

✦ ACR.mp_detect를 통해 Python에서 MediaPipe 추론 결과 데이터를 가져온 후, C++ 클라이언트에서는 다음과 같은 과정을 거쳐 처리됩니다.

1. Python 결과의 C++ 변환 (pybind11::cast)

ACR.mp_detect Python 함수가 반환하는 MediaPipe 랜드마크 결과는 Python 객체(예: List[SkeletonPoint] 또는 List[List[float]])입니다. C++ 코드에서는 pybind11 라이브러리의 cast 기능을 사용하여 이 Python 객체를 C++의 std::vector<std::vector> 타입으로 변환합니다.

- 원본 Python 데이터: [[x1, y1, z1], [x2, y2, z2], ...] 형태의 리스트
- C++ =pybind11::cast= 결과: std::vector<std::vector>

2. [@2] cv::Point3f로 재구성 및 저장

AIThread (또는 해당 로직을 수행하는 부분) 내에서 std::vector<std::vector> 타입으로 변환된 랜드마크 데이터를 순회하면서, 각 랜드마크의 x, y, z 좌표를 이용하여 OpenCV의 cv::Point3f 객체를 생성합니다. 이 cv::Point3f 객체들은 CgRPCFileClientDlg 클래스의 멤버 변수인 m_sgcp_vtSkeletonMP (std::vector)에 추가되어 저장됩니다.

- 중간 C++ 데이터: std::vector<std::vector>
- 변환 후 저장: pDlg->m_sgcp_vtSkeletonMP.push_back(cv::Point3f(landmark_data[0], landmark_data[1], landmark_data[2]));

3. [@3] 후속 C++ 로직 적용

m_sgcp_vtSkeletonMP에 저장된 스켈레톤 데이터는 AIThread가 완료 신호(hAIFinish)를 보내면 gRPCSend 스레드에서 읽어들여 후속 처리에 사용됩니다.

주요 처리 내용은 다음과 같습니다:

- =HandTurnDetector=: 스켈레톤 데이터를 기반으로 손의 움직임 변화(턴)를 감지하는 데 사용됩니다.
- =GetMotionStatusFromMP=: 손의 위치(예: READY_LOCATION)와 움직임(RAPID_DISTANCE)을 분석하여 현재 모션 상태(정지, 수어 발화, 급격 동작 등)를 판단하는 데 활용됩니다.

4. [@4] Protobuf 메시지로 직렬화 및 gRPC 전송

마지막으로, gRPCSend 스레드에서는 처리된 스켈레톤 데이터(m_sgcp_vtSkeletonMP)와 원본 이미지(ROI)를 EncodeFrame 함수를 통해 Protobuf Frame 메시지 구조로 직렬화합니다. 이 Frame 메시지에는 세션 ID, 프레임 인덱스, 플래그, 이미지 데이터, 그리고 Point3 메시지로 변환된 스켈레톤 포인트들이 포함됩니다.

이렇게 직렬화된 Protobuf 메시지는 gRPC 스트림을 통해 서버로 전송되어 최종 AI 모듈(예: 문장 인식)의 입력으로 사용됩니다.

요약하자면, =ACR.mp_detect=에서 가져온 데이터는 Python-C++ 브리징(pybind11)을 통해 C++ =std::vector<cv::Point3f>=로 변환되어 공유 메모리에 저장되고, 이후 C++ 비즈니스 로직에 활용된 다음 gRPC를 통해 서버로 스트리밍됩니다.

#+begin_quote
1번에서 python 에서 받아온 데이터는 일반적인 mediapipe의 pose 추측 결과 데이터와 동일한 타입과 크기 인가?

#+end_quote

✦ ACR.mp_detect를 통해 Python에서 받아온 데이터는 일반적인 MediaPipe의 원본 Pose 추측 결과 데이터(=mediapipe.tasks.python.vision.PoseLandmarkerResult= 객체) 와는 동일하지 않습니다. 대신, 정제되고 C++에 맞게 변환된 형태입니다.

다음과 같은 차이가 있습니다:

1. 타입 (Type): 정제된 리스트 형태

- 일반적인 MediaPipe Python API 출력: mediapipe.tasks.python.vision.PoseLandmarkerResult 객체입니다. 이 객체는 pose_landmarks, pose_world_landmarks 등 복합적인 구조와 여러 속성(x, y, z, visibility, presence)을 가집니다.
- C++에서 =pybind11::cast=를 통해 받는 데이터: std::vector<std::vector> 타입으로 예상됩니다.
  - 이는 Python ACR.mp_detect 내부에서 PoseLandmarkerResult 객체를 파싱하여 =List[List[float]]= (즉, [[x1, y1, z1], [x2, y2, z2], ...]) 형태로 재구성하여 반환한 것입니다.
  - visibility와 같은 추가 정보는 이 과정에서 제외되거나, 내부 리스트에 포함되더라도 C++에서 cv::Point3f로 변환할 때 x, y, z만 사용되므로 손실될 수 있습니다. (제안된 Python 리팩토링의 src/ai.py에서는 SkeletonPoint에 visibility를 포함했지만, _parse_landmarks가 List[SkeletonPoint]를 반환하고, 이 리스트가 C++ std::vector<std::vector>로 변환될 때는 visibility가 포함된 [x,y,z,visibility] 형태가 될 수 있습니다. 하지만 C++ 코드(cv::Point3f(landmark_data[0], landmark_data[1], landmark_data[2]))는 여전히 x, y, z만 사용합니다.)

2. [@2] 크기 (Size: 랜드마크 개수): 특정 부위만 추출되었을 가능성

- 일반적인 MediaPipe Pose Landmarker (전신): 일반적으로 33개의 랜드마크 포인트를 제공합니다.
- C++ 클라이언트가 처리하는 데이터: 기존 C++ 서버 코드(repomix-output-gRPC_server.zip.md 파일의 _LoadChangePointDB 함수)를 보면 sgcp_vtLHandSkeletonMP 및 sgcp_vtRHandSkeletonMP를 로드할 때 for (int i = 0; i < 21; i++) 루프를 사용합니다.
  - 이는 C++ 서버 측에서 21개의 손 랜드마크 데이터를 처리하고 있음을 강력하게 시사합니다.
  - 따라서 Python ACR.mp_detect 함수는 전신(33개) 랜드마크를 모두 반환하는 것이 아니라, 특정 부위(예: 왼손 21개, 오른손 21개 또는 둘 중 하나)의 랜드마크만 선택적으로 추출하여 반환했을 가능성이 높습니다.

결론

C++ 클라이언트가 Python ACR.mp_detect로부터 받는 데이터는 "일반적인 MediaPipe Pose 추측 결과 데이터"의 직접적인 원본 객체가 아니라, 이미 C++에서 사용하기 편리한 =float= 리스트의 리스트 형태로 정제되고, 랜드마크의 개수도 =손(21개)=에 특화된 형태일 가능성이 매우 높습니다. visibility 같은 부가 정보는 이 변환 과정에서 손실되거나 사용되지 않았을 것입니다.
* pybind로 주고받는 데이터 형식
=mfc_client_src/gRPCFileClientDlg.h= 와 =mfc_client_src/ACR/mp_detect.py= 를 분석한 결과, C++(MFC)와 Python(MediaPipe) 간의 데이터 교환 방식은 다음과 같습니다.
*** 1. C++ → Python (입력)
C++에서 OpenCV =cv::Mat= 객체를 Python 함수로 전달합니다. =gRPCFileClientDlg.h=에 정의된 =pybind11::detail::type_caster<cv::Mat>=를 통해 자동 변환됩니다.

- *C++ 측*: =cv::Mat dst_img= (BGR 이미지)
- *전달 방식*: =pybind11::cast(dst_img)=
- *Python 측*: =numpy.ndarray= (Shape: =[Height, Width, 3]=, dtype: =uint8=)

*** 2. Python → C++ (출력)
Python 함수 =mediapipe_pose_func= 및 =mediapipe_hand_func=는 분석 결과(랜드마크 좌표)를 =numpy.ndarray= 형태로 반환하며, 이를 C++에서 =std::vector<std::vector<double>>=로 변환하여 받습니다.

- *Python 측 반환값*: =numpy.ndarray= (dtype: =float32= 또는 =float64=)
  - *형태 (Shape)*: =(4, N)= (N=랜드마크 개수, Pose는 33개, Hand는 가변적)
  - *구성*:
    - Row 0: =all_x= (X 좌표 목록)
    - Row 1: =all_y= (Y 좌표 목록)
    - Row 2: =all_z= (Z 좌표 목록)
    - Row 3: =all_vis= (Visibility 또는 Hand Side 정보)
- *C++ 측 수신*:
  - =auto mp_result = result_pose_mp.cast<std::vector<std::vector<double>>>();=
  - 이후 =mp_result[0][k]=, =mp_result[1][k]=, =mp_result[2][k]=를 사용하여 =cv::Point3f= 객체로 재구성합니다.

*** 요약 표
| 방향          | 데이터 내용  | C++ 타입                          | Python 타입    | 비고                             |
|--------------+------------+----------------------------------+---------------+---------------------------------|
| *C++ → Python* | 입력 이미지  | =cv::Mat=                          | =numpy.ndarray= | #ERROR                          |
| *Python → C++* | 랜드마크 좌표 | =std::vector<std::vector<double>>= | =numpy.ndarray= | X, Y, Z, Visibility 순서로 행 구성 |
#+TB      LFM: $5=type_caster=로 자동 변환 (BGR)
* md_detect.py
** hand의 결과파일 landmark   자료구조
  [[id:a24739a8-75dd-4e12-8ff0-20acec42f719][landmark_자료구조]]
=mediapipe_hand_func()= 및 =mediapipe_pose_func()= 함수에 대한 상세 설명입니다.
--------------
*** =mediapipe_hand_func(img)= 함수
이 함수는 MediaPipe Hands 솔루션을 사용하여 입력 이미지에서 손 랜드마크를 감지하고, 그 정보를 정규화된 좌표 형태로 반환합니다.

*1. 초기화 (=mp_hands.Hands= 객체):*

- =mp_hands = mp.solutions.hands=: MediaPipe Hands 모듈을 불러옵니다.
- =hands_image = mp_hands.Hands(...)=: 손 감지 모델을 초기화합니다.
  - =static_image_mode=True=: 이 설정은 비디오 스트림이 아닌 단일 이미지에 대해 모델을 실행하도록 지시합니다. 모델은 각 이미지에서 손을 독립적으로 감지하므로, 추적 오버헤드가 없습니다.
  - =max_num_hands=2=: 감지할 수 있는 최대 손의 수를 2개로 설정합니다.
  - =model_complexity=1=: 랜드마크 모델의 복잡도를 설정합니다. 0은 경량 모델, 1은 기본 모델입니다. 숫자가 높을수록 정확도는 높아지지만 처리 속도는 느려집니다.
  - =min_detection_confidence=0.5=: 손 감지 모델의 최소 신뢰도 임계값을 0.5로 설정합니다. 이 값보다 낮은 신뢰도를 가진 감지는 무시됩니다.

*2. 입력 처리 및 색상 변환:*

- =rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) if (img.ndim == 3 and img.shape[2] == 3) else img=:
  - 함수는 =BGR= 이미지를 입력으로 받도록 설계되어 있습니다.
  - 만약 입력 =img=가 3채널(높이 x 너비 x 3) BGR 이미지라면, =cv2.cvtColor()=를 사용하여 =RGB= 형식으로 변환합니다.
  - 이미 =RGB= 형식으로 제공되었거나 다른 형식이라면 그대로 사용합니다. MediaPipe 모델은 =RGB= 이미지를 기대합니다.

*3. 손 랜드마크 감지:*

- =results = hands_image.process(rgb)=: 초기화된 =hands_image= 모델의 =process()= 메서드를 사용하여 =RGB= 이미지에서 손 랜드마크를 감지합니다. 이 =results= 객체에는 감지된 손의 랜드마크(=multi_hand_landmarks=) 및 각 손의 방향 정보(=multi_handedness=)가 포함됩니다.

*4. 랜드마크 정보 추출 및 정렬:*

- =all_x, all_y, all_z, all_side = [], [], [], []=: 감지된 손 랜드마크의 x, y, z 좌표와 손의 방향(오른손/왼손)을 저장할 리스트를 초기화합니다.
- =if results.multi_hand_landmarks:=: 손 랜드마크가 감지된 경우에만 다음 로직을 실행합니다.
- =packs= 리스트에 각 손의 방향(=side=), 신뢰도(=score=), 랜드마크 목록(=hlm.landmark=)을 튜플 형태로 저장합니다. =multi_handedness=를 통해 손의 방향(예: "Right", "Left")과 해당 감지의 신뢰도를 가져옵니다.
- =priority = {"Right": 1, "Left": 0}=: 손 정렬을 위한 우선순위 딕셔너리를 정의합니다. 'Right' 손이 'Left' 손보다 우선순위가 높습니다.
- =packs.sort(key=lambda t: (priority.get(t[0], 2), -t[1]))=: 감지된 손들을 정렬합니다.
  - *주요 정렬 기준:* =priority.get(t[0], 2)=를 사용하여 'Right' 손을 먼저, 그 다음 'Left' 손을 정렬합니다. 알 수 없는 손은 가장 뒤로 갑니다.
  - *보조 정렬 기준:* =-t[1]=을 사용하여 같은 종류의 손(예: 모두 'Right' 손) 내에서는 신뢰도 점수(=score=)가 높은 순서대로 정렬합니다. 이 정렬은 출력 순서의 일관성을 보장합니다.
- 정렬된 =packs= 리스트를 순회하며 각 손의 21개 랜드마크에 대해 x, y, z 좌표를 =all_x=, =all_y=, =all_z= 리스트에 추가합니다.
- =all_side= 리스트에는 'Right' 손인 경우 =1.0=, 'Left' 손인 경우 =0.0=을 추가하여 손의 방향 정보를 인코딩합니다.

*5. 반환 값:*

- =return np.array([all_x, all_y, all_z, all_side], dtype=np.float32)=: 수집된 모든 x, y, z 좌표와 손의 방향 정보를 NumPy 배열로 변환하여 반환합니다. 이 배열은 각 손 랜드마크의 정규화된 3D 좌표 (0~1 범위)와 해당 랜드마크가 어느 손에 속하는지 나타내는 정보를 포함합니다. (예: =[x_coords], [y_coords], [z_coords], [side_info]=)

--------------

*** =mediapipe_pose_func(img)= 함수
이 함수는 MediaPipe Pose 솔루션을 사용하여 입력 이미지에서 인체 포즈 랜드마크를 감지하고, 그 정보를 정규화된 좌표와 가시성 형태로 반환합니다.

*1. 초기화 (=mp_pose.Pose= 객체):*

- =mp_pose = mp.solutions.pose=: MediaPipe Pose 모듈을 불러옵니다.
- =pose = mp_pose.Pose(...)=: 포즈 감지 모델을 초기화합니다.
  - =static_image_mode=False=: 이 설정은 원래 비디오 스트림에서 객체 추적을 활성화하지만, 현재 함수 내에서는 단일 이미지에 대해 =process()=를 호출하고 있으므로 실제 추적 이점은 활용되지 않습니다. 모델은 각 이미지에서 포즈를 독립적으로 감지합니다.
  - =model_complexity=1=: 랜드마크 모델의 복잡도를 설정합니다 (0: 경량, 1: 기본, 2: 풀 모델).

*2. 입력 처리 및 색상 변환:*

- =rgb_frame = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)=: 입력 =img=가 BGR 이미지로 간주되므로, =cv2.cvtColor()=를 사용하여 =RGB= 형식으로 변환합니다. MediaPipe 모델은 =RGB= 이미지를 기대합니다.

*3. 포즈 랜드마크 감지:*

- =results = pose.process(rgb_frame)=: 초기화된 =pose= 모델의 =process()= 메서드를 사용하여 =RGB= 이미지에서 포즈 랜드마크를 감지합니다. 이 =results= 객체에는 감지된 포즈 랜드마크(=pose_landmarks=)가 포함됩니다.

*4. 랜드마크 정보 추출:*

- =all_x, all_y, all_z, all_vis = [], [], [], []=: 감지된 포즈 랜드마크의 x, y, z 좌표와 가시성(visibility)을 저장할 리스트를 초기화합니다.
- =if results.pose_landmarks:=: 포즈 랜드마크가 감지된 경우에만 다음 로직을 실행합니다.
- =for lm in results.pose_landmarks.landmark:=: 감지된 모든 포즈 랜드마크(=results.pose_landmarks.landmark=)를 순회합니다.
  - 각 랜드마크(=lm=)에서 정규화된 x, y, z 좌표와 =visibility= (랜드마크가 이미지에 얼마나 잘 보이는지 나타내는 값, 0.0에서 1.0 사이)를 추출하여 해당 리스트에 추가합니다.

*5. 반환 값:*

- =return np.array([all_x, all_y, all_z, all_vis])=: 수집된 모든 x, y, z 좌표와 가시성 정보를 NumPy 배열로 변환하여 반환합니다. 이 배열은 각 포즈 랜드마크의 정규화된 3D 좌표 (0~1 범위)와 해당 랜드마크의 가시성 정보를 포함합니다.
*

 [[id:527735bd-c383-4786-8a6a-14bf471964ca][landmark_자료구조이해]]
** pose 의 결과파일 landmark 자료구조
 *** 요약
 - 33개의 항목에 대한 (x,y,z,visibility) 으로 구성.
 - 33개 항목  세부내용
   [[id:84040e98-d126-4eae-b776-737b0b4e9415][pose landmark 요소]]
MediaPipe Pose 랜드마크의 세부 자료형과 예제 숫자는 다음과 같습니다.

*** 랜드마크 세부 자료형
=mediapipe_pose_func= 함수에서 =results.pose_landmarks.landmark=를 통해 접근하는 각 =lm= (랜드마크) 객체의 속성들은 다음과 같은 자료형을 가집니다:

- =lm.x=  : =float= (부동 소수점 숫자)
- =lm.y= : =float= (부동 소수점 숫자)
- =lm.z= : =float= (부동 소수점 숫자)
- =lm.visibility= : =float= (부동 소수점 숫자)

결과적으로 이들을 모아 =np.array= 로 반환할 때 =dtype= 를 따로 지정하지 않았으므로, NumPy는 내부적으로 가장 적절한 부동 소수점 타입을 선택합니다. (일반적으로 =float64=가 될 수 있습니다, 하지만 MediaPipe 자체는 =float=를 반환합니다.)

*** 예제 숫자
랜드마크 값들은 이미지의 너비와 높이에 대해 *0.0에서 1.0 사이로 정규화된 값*입니다. =z= 값은 상대적인 깊이를 나타내며 음수 또는 양수 값을 가질 수 있습니다. =visibility= 또한 0.0에서 1.0 사이의 값으로 표현됩니다.

예를 들어, 한 랜드마크(예: 코)에 대한 값은 다음과 같을 수 있습니다:

#+begin_example
lm = {
    'x': 0.485,        # 이미지 너비의 48.5% 지점 (중앙에 가까움)
    'y': 0.210,        # 이미지 높이의 21.0% 지점 (상단에 가까움)
    'z': -0.052,       # 상대적인 깊이. (코는 어깨보다 몸 밖으로 나와있는 경향이 있어 음수값을 가질 수 있습니다)
    'visibility': 0.98 # 매우 잘 보이는 랜드마크
}
#+end_example

*전체 =np.array= 반환 형태 예시 (단순화된 3개 랜드마크 가정):*

실제로는 33개의 랜드마크가 있지만, 이해를 돕기 위해 3개의 랜드마크만 있다고 가정하고 =mediapipe_pose_func=의 반환 형태를 보면:

#+begin_src python
# 예를 들어, 코, 왼쪽 어깨, 오른쪽 어깨 랜드마크가 감지되었다고 가정
[
    [0.485, 0.350, 0.620],  # all_x: 각 랜드마크의 x 좌표
    [0.210, 0.400, 0.410],  # all_y: 각 랜드마크의 y 좌표
    [-0.052, 0.120, 0.150], # all_z: 각 랜드마크의 z 좌표
    [0.980, 0.950, 0.960]   # all_vis: 각 랜드마크의 가시성
]
#+end_src

여기서 각 내부 리스트는 NumPy 배열의 한 행이 되며, 각 열은 해당 랜드마크의 특정 속성(x, y, z, visibility) 값을 나타냅니다.
* C++ 과  python keyFrame  검출 차이
** 전체적 비교
두 프로젝트(=@mfc_client_src=, =@ksl-cli=)의 KeyFrame 검출 로직은 *"손의 방향 전환(Turn)이 감지된 후, 움직임이 멈추는(Stabilized) 순간"*을 포착한다는 핵심 알고리즘을 공유하고 있습니다.

Python 버전은 C++ MFC의 로직을 그대로 포팅하였기에 논리적 흐름은 99% 일치합니다.

상세 비교 내용은 다음과 같습니다.

--------------

*** 1. KeyFrame 검출 핵심 알고리즘 (공통)
두 프로젝트 모두 아래의 *4단계 조건*이 모두 충족될 때 해당 프레임을 KeyFrame으로 전송합니다.

1. *Hand Status Check (사전 조건):* 손이 준비 자세(아래로 내린 상태)가 아닌, 수어 공간(위쪽)에 올라와 있어야 함.
2. *Hand Turn Detection (이벤트 트리거):* 손목의 이동 방향이 급격히 바뀌면서 속도가 줄어드는 "터닝 포인트"가 감지되었는가? (=speak_start_flag = True=)
3. *Optical Flow Stability (전송 시점):* 화면 전체(혹은 ROI)의 픽셀 움직임 평균값(=avgMotion=)이 특정 임계값(=0.45=) 이하로 떨어졌는가?
4. *Refractory Period (중복 방지):* 직전 KeyFrame 전송 후 일정 시간(=frozen_flag == 0=)이 지났는가?

--------------

*** 2. 코드 레벨 상세 비교
**** A. Motion & Hand Status (준비 상태 확인)
- *Logic:* MediaPipe의 3D 좌표를 2D 평면(1920x1080 기준)으로 Affine 변환하여, 손목의 Y좌표가 =READY_LOCATION(700)=보다 위에 있는지(화면상 상단) 확인합니다.
- *C++ (=gRPCFileClientDlg.cpp=):*
  - =GetRefHL2DPointsMP=: 어깨와 코 좌표를 기준으로 Affine Transform 수행.
  - =GetRoughHandStatusFromMP=: 변환된 좌표가 700보다 작은지(위쪽인지) 확인.
- *Python (=src/logic.py=):*
  - =get_ref_hl_2d_points_mp=: C++의 Affine 변환 로직을 =cv2.getAffineTransform=으로 동일하게 구현.
  - =get_rough_hand_status_from_mp=: 동일하게 정규화된 Y 좌표 비교.

**** B. Turn Detection (손목 꺾임 감지)
- *Logic:* 손목의 벡터 내적(Dot Product)을 통해 각도 변화(=angle=)를 계산하고, 이전 속도 대비 현재 속도의 비율(=ratio=)로 감속 여부를 확인합니다.
  - 조건: =각도 > 30도= AND =속도비율 < 0.8= (방향이 꺾이면서 감속함)
- *C++ (=HandTurnDetector.hpp=):*
  - 클래스 내부에서 =update()= 호출 시 =prevVel_=, =prevPos_=를 갱신하며 계산.
  - =gRPCThread_.h= 루프 내에서 =m_Rdetector.update(...)= 호출.
- *Python (=src/logic.py= & =src/main.py=):*
  - =HandTurnDetector= 클래스로 =HandTurnDetector.hpp=를 1:1 포팅.
  - =numpy=를 사용하여 벡터 연산 수행. =update= 메서드 로직 일치.

**** C. Optical Flow (화면 정지 감지)
- *Logic:* Lucas-Kanade Optical Flow를 사용하여 이전 프레임과 현재 프레임 사이의 특징점 이동 거리 평균(=avgMotion=)을 계산합니다.
- *C++ (=VideoUtil-v1.1.cpp= - =GetMotionVauleWithOpticalFlow=):*
  - =cv::goodFeaturesToTrack= -> =cv::calcOpticalFlowPyrLK=
  - 이동한 픽셀 거리의 평균을 반환.
- *Python (=src/video.py= - =calculate_optical_flow_value=):*
  - =cv2.goodFeaturesToTrack= -> =cv2.calcOpticalFlowPyrLK=
  - 동일한 파라미터(=maxCorners=300=, =qualityLevel=0.01= 등) 사용.

**** D. 최종 KeyFrame 판정 (The Trigger)
이 부분이 가장 중요하며, 두 코드에서 *변수명까지 거의 유사하게 매칭*됩니다.

*C++ (=gRPCThread_.h=):*

#+begin_src cpp
// 1. 턴 감지 시 플래그 세팅
if (Rhit) { speakstartflag = 1; }
else if (Lhit) { speakstartflag = 1; }

// 2. 전송 조건 검사
// speakstartflag: 이전에 턴이 발생했었음
// avgMotion < OPTICALFLOW_THRESH: 화면이 정지함
// frozenflag == 0: 쿨타임 끝남
...
# 2. 전송 조건 검사
# speak_start_flag: 이전에 턴이 발생했었음
# avg_motion < OPTICAL_FLOW_THRESH: 화면이 정지함
# frozen_flag == 0: 쿨타임 끝남
# hand_status != 0: 손이 내려가 있지 않음 (C++도 상위 if문에서 체크함)
# not is_turn_detected: 현재 턴하는 중이 아님 (C++은 else if 구조로 이를 암시적으로 처리)
...
*** 3. 미세한 차이점 및 결론
1. *제어 구조:*
   - *C++:* =if (Rhit) ... else if (Lhit) ... else if (SendCondition)= 구조입니다. 즉, 턴이 감지되는 *당일 프레임*에는 절대 KeyFrame을 보내지 않습니다.
   - *Python:* =if is_turn_detected: ...= 후에 =if ... and not is_turn_detected:= 조건을 명시하여 C++의 =else if= 논리를 그대로 구현했습니다.
2. *데이터 타입:*
   - *C++:* OpenCV의 =cv::Point2f= 등을 사용.
   - *Python:* Numpy 배열 및 튜플 사용. 하지만 수학적 계산 결과는 동일합니다.

*결론:* =@ksl-cli= 프로젝트는 =@mfc_client_src=의 KeyFrame 검출 로직(상수 값 =0.45=, =3=, =700= 등 포함)을 *정확하게 모사(Porting)* 하였습니다. 따라서 입력 비디오가 동일하다면, 검출되는 KeyFrame의 타이밍과 개수는 *거의 동일*할 것으로 예상됩니다.
** 세부 코드 비교
두 프로젝트의 KeyFrame 검출 로직은 *4단계 조건*이 논리적으로 완벽하게 일치합니다. 각 단계별로 C++ 원본 코드와 Python 포팅 코드를 비교하여 분석해 드립니다.

--------------

*** 1단계: Hand Status Check (손의 위치 확인)
손이 "준비 자세(아래)"가 아니라 "수어 공간(위)"에 있는지 확인합니다.

- *비교:* 두 코드 모두 어깨와 코의 좌표를 기준으로 *Affine 변환*을 수행하여 정규화된 좌표를 얻고, 손목의 Y좌표가 특정 임계값(=READY_LOCATION=)보다 위에 있는지(화면 좌표계상 값이 작은지) 판단합니다.

**** C++ (=gRPCFileClientDlg.cpp=)
#+begin_src cpp
// 1080p 기준 700픽셀 라인보다 위에 있어야 함 (Y좌표가 작아야 함)
#define READY_LOCATION 700

int CgRPCFileClientDlg::GetRoughHandStatusFromMP(std::vector<Point3f> mp_pose) {
    // Affine 변환을 통해 정규화된 손목 좌표(hl) 획득
    auto hl = GetRefHL2DPointsMP(mp_pose);

    // 왼쪽/오른쪽 손목의 Y좌표가 READY_LOCATION(700)보다 작거나 같은지 확인
    if (hl[1].y <= READY_LOCATION && hl[0].y <= READY_LOCATION) status = 3; // Both
    // ... (생략)
    else status = 0; // Ready (손이 아래에 있음)
    return status;
}
#+end_src
...
def get_rough_hand_status_from_mp(mp_pose: List[SkeletonPoint]) -> int:
    ...
    # C++ 로직과 동일하게 1080p 기준 비율로 정규화하여 비교
    normalized_ready_location = READY_LOCATION / 1080.0

    left_hand_y = hl_points[0][1]
    right_hand_y = hl_points[1][1]

    # Y좌표가 임계값보다 작거나 같은지(위쪽인지) 확인
...
*** 2단계: Hand Turn Detection (손목 방향 전환 감지)
손목이 이동하다가 방향을 급격히 틀면서(=angle=) 속도가 줄어드는(=speed ratio=) 순간을 감지합니다.

- *비교:* 벡터의 내적(Dot Product)을 이용한 각도 계산과 속도 비율 계산 로직이 수학적으로 동일합니다.

**** C++ (=HandTurnDetector.hpp=)
#+begin_src cpp
// 각도 > 30도 AND 속도비율 < 0.8 (감속)
if (dtheta > angleRadTh_ && ratio < speedRatioTh_) {
    detected = true;
}
#+end_src
...
**** Python (=src/logic.py=)
#+begin_src python
# C++ 생성자 파라미터 (30.0, 0.8)과 동일한 기본값 사용
def __init__(self, angle_deg_th: float = 30.0, speed_ratio_th: float = 0.8, ...):
    # ...
...
*** 3단계: Optical Flow Stability (화면 움직임 안정화 확인)
손이 멈추거나 움직임이 거의 없는 순간(정적 구간)을 포착하기 위해 화면 전체의 픽셀 변화량을 측정합니다.

- *비교:* OpenCV의 =calcOpticalFlowPyrLK= 함수를 사용하며, 판단 기준인 임계값(=0.45=)이 동일합니다.

**** C++ (=gRPCThread_.h= & =VideoUtil-v1.1.cpp=)
#+begin_src cpp
#define OPTICALFLOW_THRESH 0.45

// VideoUtil-v1.1.cpp
// GoodFeaturesToTrack -> CalcOpticalFlowPyrLK 수행 후 이동거리 평균 리턴
...
**** Python (=src/main.py= & =src/video.py=)
#+begin_src python
OPTICAL_FLOW_THRESH = 0.45

# src/video.py
# cv2.calcOpticalFlowPyrLK 수행 후 이동거리 평균 리턴
...
*** 4단계: Final Trigger Condition (최종 전송 판정)
위의 조건들을 종합하여 실제로 서버에 프레임을 보낼지 결정합니다.

- *비교:*
  1. *Turn 발생 기록 (=speak_start_flag=)*: 과거에 손목 꺾임이 있었는가?
  2. *화면 정지 (=avgMotion=)*: 현재 화면이 멈췄는가?
  3. *쿨타임 (=frozen_flag=)*: 방금 전송하지 않았는가?
  4. *현재 턴 아님*: C++은 =else if= 구조로, Python은 =not is_turn_detected=로 동일하게 처리함.

**** C++ (=gRPCThread_.h=)
#+begin_src cpp
// 1. 턴 감지 시 플래그 설정 (이때는 전송 안 함)
if (Rhit) { speakstartflag = 1; }
else if (Lhit) { speakstartflag = 1; }

// 2. 턴이 아닐 때(else if), 나머지 조건 확인하여 전송
else if (speakstartflag && avgMotion < OPTICALFLOW_THRESH && frozenflag == 0)
{
    writer->Write(frame); // 전송
    frozenflag = OPTICAL_FLOW_HOLD_FRAME; // 쿨타임(3프레임) 설정
}
#+end_src
...
# 2. 전송 조건 확인
# hand_status != 0: 손이 내려가지 않음
# not is_turn_detected: 현재 턴하는 중이 아님 (C++의 else if 역할)
if speak_start_flag and avg_motion < OPTICAL_FLOW_THRESH and frozen_flag == 0 and hand_status != 0 and not is_turn_detected:
    should_send_keyframe = True # 전송 플래그
    frozen_flag = OPTICAL_FLOW_HOLD_FRAME # 쿨타임(3프레임) 설정
#+end_src

*** 요약
C++ 코드의 알고리즘 흐름과 상수 값(=700=, =30.0=, =0.8=, =0.45=, =3=)이 Python 코드로 *정확하게 1:1 이식*되었습니다. 따라서 두 클라이언트는 동일한 비디오 입력에 대해 *거의 동일한 타이밍의 KeyFrame을 검출*하게 됩니다.
* python code 실행 과정과 시간 유추
`ksl-cli`는 **FHD (1920x1080)** 원본 영상을 입력받더라도, 내부 로직에 의해 **가로 320px**로 리사이징된 데이터를 주로 처리하고 전송합니다.

다음은 데이터 흐름에 따른 각 단계별 함수와 처리되는 데이터의 크기 분석입니다.
*(가정: ROI를 설정하지 않고 1920x1080 전체를 처리한다고 가정할 때)*
...
- frame range:  669- 835
- 추출 keyframe 개수: 15
*** 집계 대상: `[ETA_LOG] 1~4 단계`

| 단계 | 항목          | 샘플 수 | 평균 (sec) | 평균 (ms) | 중앙값 (ms) | 최소~최대 (ms)    |
|  -: | ------------ |   ---: |  -------: |  ------: |   -------: | --------------: |
|   1 | VideoLoading |    836 |  0.001082 |    1.082 |      0.828 | 0.771 ~ 37.046  |
|   2 | PreProcessed |    836 |  0.000057 |    0.057 |      0.056 | 0.043 ~ 0.191   |
|   3 | Optical Flow |     73 |  0.001527 |    1.527 |      1.454 | 1.225 ~ 2.751   |
|   4 | AI 추론       |    167 |  0.039057 |   39.057 |     39.712 | 24.533 ~ 55.249 |


  * 단계별 샘플 수 불일치
  * 3단계 73회, 4단계 167회, 1~2단계 836회
  * 1단계 최대값 37.046ms 한 건, 초기 1회성 지연 가능성
** perf stat
- perf 명령어
1. 기본 stat
    Performance counter stats for 'python ./src/main.py ../5.mp4 740,377,266,300 --model-path ksl-cli/protos/pose_landmarker_full.task --log-level DEBUG --frame 669 835 --keyframeOnly 2':

         16,361.93 msec task-clock                       #    1.387 CPUs utilized
            28,961      context-switches                 #    1.770 K/sec
             4,830      cpu-migrations                   #  295.197 /sec
           175,594      page-faults                      #   10.732 K/sec
   150,126,221,462      instructions                     #    2.26  insn per cycle
                                                  #    0.04  stalled cycles per insn
    66,303,092,598      cycles                           #    4.052 GHz
     6,698,014,455      stalled-cycles-frontend          #   10.10% frontend cycles idle
    12,404,349,146      branches                         #  758.123 M/sec
       330,428,647      branch-misses                    #    2.66% of all branches

      11.799065668 seconds time elapsed

      13.981134000 seconds user
       2.456133000 seconds sys

2. 단계별 코드 블럭 측정
3. instruction 기준 측정
   Performance counter stats for 'python ./src/main.py ../5.mp4 740,377,266,300 --model-path ksl-cli/protos/pose_landmarker_full.task --log-level DEBUG --frame 669 835 --keyframeOnly 2' (5 runs):

   141,600,871,063      instructions                     #    2.67  insn per cycle              ( +-  0.01% )
    53,099,250,580      cycles                                                                  ( +-  0.50% )
       706,261,092      cache-misses                                                            ( +-  0.19% )

           12.9878 +- 0.0724 seconds time elapsed  ( +-  0.56% )
** jetson nano orion 비교 예상
아래 비교는 **현재 측정값(Ryzen 5 5600)** 을 기준으로
**Jetson Orin Nano**에서 실행했을 때를 **정량적으로 추정·해석**한 것이다.
...
```text
Ryzen 5600 ≈ 141.6B instructions
Orin Nano  ≈ 141.6B instructions
```
...
```text
Ryzen 성능 기준 = 2.67 × 4.2 = 11.21
Orin 성능 기준  = 1.8 × 1.6 = 2.88
```
...
**** CPU 기준
  ...
  * Jetson Orin Nano는
    ...
    * Ryzen 5600 대비 **약 4배 느림**
**** 성능 개선 방향

  * CPU 최적화 ❌ (이미 한계)
  * **AI 추론 GPU/DLA 이전 필수**
  * Optical Flow / Preprocess 병렬화 필요

---

## 한 줄 요약

**** **같은 코드를 CPU로만 돌리면 Orin Nano는 약 50초대**
**** **Orin의 가치는 CPU가 아니라 GPU·DLA 활용에 있음**
*** 표로 정리
| 항목                | AMD Ryzen 5 5600 (PC) | Jetson Orin Nano     | 의미                    |
| ----------------   | --------------------- | -------------------- | ---------------        |
| CPU 아키텍처         | x86-64 (Zen 3)        | ARMv8 (Cortex-A78AE) | 명령어 집합·파이프라인 차이 |
| 코어 / 스레드        | 6C / 12T              | 6C / 6T              | SMT 유무                |
| 평균 동작 클럭        | ~4.2 GHz              | ~1.6 GHz             | 실효 클럭 차이            |
| 실효 IPC            | ~2.6 ~ 2.8            | ~1.6 ~ 2.0           | 사이클당 처리량           |
| CPU 단일코어 성능 지수 | 11.2                  | 2.9                  | IPC × Clock           |
| 실행 instruction 수 | **141.6B**              | **141.6B (동일)**       | 연산 “양”               |
| 사용 cycles (추정)   | 53.1B                 | ~80 ~ 90B            | IPC·클럭 영향           |
| 평균 실행 시간        | **12.99 s**             | **45 ~ 60 s (추정)**    | CPU-only               |
| 상대 실행 시간        | 1×                   | **~3.9× 느림**         | CPU 기준                |
| 캐시 미스 영향        | 낮음                   | 낮음                  | 메모리 병목 아님          |
| CPU 병목 여부        | 있음                   | 있음                  | 연산 중심                |
***
아래는 **이번 비교에 사용한 `perf` 명령어들을 org-mode 형식으로 정리**한 내용이다.
요청대로 **노드 레벨(계층) 유지**.

---

* perf 명령어 정리 (CPU 비교 목적별)

** 1. 전체 CPU 연산 로드 비교 (핵심 기준)
*** 목적

  * CPU 종류와 무관한 *절대 연산량(workload)* 비교
  * 다른 CPU에서의 실행 시간 추정 기준 확보
*** 명령어
  #+begin_src bash
...
*** 핵심 지표
  ...
  * instructions :: CPU가 실제 수행한 명령 총량
  * cycles :: 실행에 사용된 클럭 사이클 수
*** 의미

  * instructions 값이 같으면 “같은 일을 했다”
  * CPU 비교의 출발점
...
*** 목적
  ...
  * 동일한 연산량을 CPU가 얼마나 효율적으로 처리했는지 판단
*** 명령어
  #+begin_src bash
  perf stat -e instructions,cycles python ./src/main.py <args...>
  #+end_src
*** 계산 지표
...
*** 목적
  ...
  * 성능 저하 원인이 CPU 연산인지 메모리 접근인지 구분
*** 명령어
  #+begin_src bash
...
*** 명령어
  #+begin_src bash
  perf stat -r 5 python ./src/main.py <args...>
  #+end_src
*** 의미

  * 스케줄링, I/O, thermal 영향 감소
  * Jetson 계열에서 특히 중요

---

** 5. CPU 코어 고정 (공정 비교 필수)
...
*** 명령어
  #+begin_src bash
  taskset -c 0 perf stat python ./src/main.py <args...>
  #+end_src
*** 의미
...
*** 명령어
  #+begin_src bash
  perf record python ./src/main.py <args...>
  perf report
  #+end_src
*** 의미
  * Python 인터프리터 내부 vs C/CUDA 코드 구분
  * GPU/DLA 오프로딩 판단 근거
